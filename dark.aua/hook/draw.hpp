...
namespace apn::dark::hook
{
	...
	inline struct Draw
	{
		...
		BOOL init()
		{
			...
			my::addr_t address1 = ::CallWindowProcW((WNDPROC)get_ret_addr, 0, 0, 0, 0) - 0x2B;
			{
				BYTE code[1] = {};
...
//			my::hook::attach(PatBlt);
		...
		struct {
			inline static BOOL WINAPI hook_proc(HDC dc, int left, int top, int right, int bottom)
			{
				...
				MY_TRACE_FUNC("{:#010x}, {:#010x}, {}, {}, {}, {}", from, dc, left, top, right, bottom);
		...
		struct {
			...
			{
				auto from = *((my::addr_t*)&dc - 1);
				MY_TRACE_FUNC("{:#010x}, {:#010x}, ({}), {}, {:#010x}", from, dc, safe_string(rc), width, type);
		...
		struct {
			...
			{
				auto from = *((my::addr_t*)&dc - 1);
				MY_TRACE_FUNC("{:#010x}, {:#010x}, ({}), {:#010x}, {:#010x}", from, dc, safe_string(rc), type, state);
		...
		struct {
			...
			{
				auto from = *((my::addr_t*)&dc - 1);
				MY_TRACE_FUNC("{:#010x}, {:#010x}, ({}), {:#010x}, {:#010x}", from, dc, safe_string(rc), edge, flags);
		...
		struct {
			...
			{
				auto from = *((my::addr_t*)&dc - 1);
				MY_TRACE_FUNC("{:#010x}, {:#010x}, ({})", from, dc, safe_string(rc));
		...
		struct {
			...
			{
				auto from = *((my::addr_t*)&dc - 1);
				MY_TRACE_FUNC("{:#010x}, {:#010x}, {:#010x}, {}, {}, {}, {}, {:#010x}", from, dc, fore, x, y, cx, cy, flags);
...
//					MY_TRACE_FUNC("再帰呼び出し時はフックしません\n");
				...
				MY_TRACE_FUNC("{:#010x}, {:#010x}, {}, {}, {:#010x}, ({}), {}",
					from, dc, x, y, options, safe_string(rc), safe_string(text, c, options));
		...
		struct {
			...
			{
				auto from = *((my::addr_t*)&dc - 1);
				MY_TRACE_FUNC("{:#010x}, {:#010x}, {}, {}, {}, {}, {:#010x}", from, dc, x, y, w, h, rop);
