...
{
	//
	// このクラスはショートカットキーを保持します。
	//
	struct ShortcutKey
	{
		struct KeyCode {
			inline static constexpr LabelT<int> labels[] = {
		...
		struct Modifier {
			inline static const DWORD Shift = 0x01;
			inline static const DWORD Ctrl = 0x02;
			inline static const DWORD Alt = 0x04;
			inline static const DWORD Win = 0x08;
		...
		// コンストラクタです。
		//
		ShortcutKey(int vk, DWORD modifier)
			: vk(vk)
			, modifier(modifier)
		{
		}
		...
		// コンストラクタです。
		//
		ShortcutKey(LPCWSTR vk, LPCWSTR modifier)
			: vk(str2vk(vk))
			, modifier(str2mod(modifier))
		{
		}
		...
		//
		BOOL isActive(int vk)
		{
			if (vk != this->vk) return FALSE;
			if (modifier & Modifier::Shift && !isKeyDown(VK_SHIFT)) return FALSE;
			if (modifier & Modifier::Ctrl && !isKeyDown(VK_CONTROL)) return FALSE;
			if (modifier & Modifier::Alt && !isKeyDown(VK_MENU)) return FALSE;
			if (modifier & Modifier::Win && !isKeyDown(VK_LWIN, VK_RWIN)) return FALSE;
		...
		//
		template <class... Tail>
		static BOOL isKeyDown(int vk, Tail&&... tail)
		{
			if (isKeyDown(vk))
				return TRUE;
			else
				return isKeyDown(std::forward<Tail>(tail)...);
		}

		//
		// 文字列を仮想キーコードに変換して返します。
		//
		static int str2vk(LPCWSTR _s)
		{
			std::wstring s(_s);
			std::transform(s.begin(), s.end(), s.begin(), toupper);

			for (auto label : KeyCode::labels)
			{
				if (s == label.text)
					return label.value;
			}
		...
		// 文字列を修飾子に変換して返します。
		//
		static DWORD str2mod(LPCWSTR _s)
		{
			std::wstring s(_s);
			std::transform(s.begin(), s.end(), s.begin(), toupper);
			...
			for (auto label : Modifier::labels)
			{
		...
		//
		static std::wstring vk2str(int vk)
		{
			for (auto label : KeyCode::labels)
			{
				if (vk == label.value)
		...
		// 修飾子を文字列に変換して返します。
		//
		static std::wstring mod2str(DWORD modifier)
		{
			std::wstring s(L"");

			for (auto label : Modifier::labels)
			{
				if (modifier & label.value)
				{
					if (s.length()) s += L" ";
					s += label.text;
				}
