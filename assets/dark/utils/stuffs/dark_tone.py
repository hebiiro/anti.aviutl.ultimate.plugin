...
# ダークトーンスタッフを作成します。
#
def create_dark_tone_stuffs(fill_tones, border_tones, text_tones):
	# トーンを登録しておきます。
	...
	if (stuff):
		stuff.text.color = primary_text(+0.8)
	...
	if (stuff):
		...
		stuff.fill.sub_color = secondary_fill(-0.4)
		stuff.fill.mode = fill_mode
		stuff.text.color = secondary_text(0.9)
		stuff.text.shadow_color = text_shadow(stuff)
		stuff.border.color = secondary_border(-0.8)
	...
	if (stuff):
		...
		stuff.fill.sub_color = primary_fill(0.1)
		stuff.fill.mode = fill_mode
		stuff.border.color = primary_border(-0.8)
		stuff.border.width = 1
		stuff.etc.ellipse = ellipse
	...
	if (stuff):
		stuff.fill.color = secondary_fill(0.3)
	...
	if (stuff):
		stuff.fill.color = secondary_fill(0.1)
		stuff.fill.sub_color = secondary_fill(-0.2)
		stuff.fill.mode = fill_mode
		stuff.border.color = secondary_border(-0.9)
		stuff.border.width = 1
	...
	if (stuff):
		stuff.fill.color = primary_fill(0.3)
		stuff.fill.sub_color = primary_fill(-0.2)
		stuff.fill.mode = fill_mode
		stuff.border.color = primary_border(-0.9)
		stuff.border.width = 1
		stuff.etc.ellipse = ellipse
	...
	if (stuff):
		stuff.fill.color = primary_fill(-0.2)
	...
	icon.change_color("", CLR(0), primary_text(0.8))
