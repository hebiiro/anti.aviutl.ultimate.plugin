...
# 『[旧形式] astro』のライトモードの設定です。
# はくしんさんの『Vivid Light』を元に作成しています。
...
if (stuff):
	stuff.fill.color = CLR("#E5E5E5")
	stuff.text.color = CLR("#444444")
	stuff.border.color = CLR("#2B9FFE")
	stuff.border.width = 1

## ウィンドウキャプションの非アクティブ状態
stuff = utils.create_stuff("window_caption_inactive")
if (stuff):
	stuff.fill.color = CLR("#EBEBEB")
	stuff.text.color = CLR("#686868")
	stuff.border.color = CLR("#0038FF")
	stuff.border.width = 1
...
if (stuff):
	stuff.fill.color = CLR("#D9EEFF")
	stuff.text.color = CLR("#048EFF")
	stuff.text.shadow_color = text_shadow(stuff)
...
if (stuff):
	stuff.fill.color = CLR("#C8C8C8")
	stuff.border.color = CLR("#444444")
	stuff.border.width = 1
	stuff.etc.ellipse = ellipse
...
if (stuff):
	stuff.fill.color = CLR("#E1E1E1")
	stuff.border.color = CLR("#C6C6C6")
	stuff.border.width = 1
...
if (stuff):
	stuff.fill.color = CLR("#AAAAAA")

## 奇数レイヤー背景のアクティブ状態
stuff = utils.create_stuff("exedit_odd_layer_background_active")
if (stuff):
	stuff.fill.color = ADD(CLR("#FAFAFA"), -7)
