...
# 『[旧形式] ライム』のライトモードの設定です。

#
# 指定されたキーカラーを補正して返します。
#
def KEY(color, add):
	return ADD(color, add, 0, add)
...
# よく使う配色を定義します。
#
normal_fill_color = CLR(240, 255, 240)
normal_text_color = CLR(0, 0, 0)
normal_border_color = CLR(100, 200, 100)
disabled_fill_color = CLR(250)
disabled_text_color = CLR(120)
disabled_border_color = CLR(150)
key_fill_color = CLR(220, 255, 220)
key_text_color = CLR(0, 0, 0)
key_border_color = CLR(100, 200, 100)
...
if (stuff):
	...
	stuff.border.color = KEY(key_fill_color, -20)
	stuff.border.width = 1
...
if (stuff):
	stuff.fill.color = KEY(normal_fill_color, -20)
	stuff.text.color = normal_text_color
...
if (stuff):
	...
	stuff.text.color = KEY(key_text_color, 0)
	stuff.text.shadow_color = text_shadow(stuff)
	stuff.border.color = KEY(key_border_color, -30)
	stuff.border.width = border_width
	stuff.etc.ellipse = ellipse
...
if (stuff):
	...
	stuff.text.color = KEY(key_text_color, 0)
	stuff.text.shadow_color = text_shadow(stuff)
	stuff.border.color = KEY(key_border_color, -50)
	stuff.border.width = border_width
	stuff.etc.ellipse = ellipse
...
if (stuff):
	stuff.fill.color = KEY(normal_fill_color, -5)
	stuff.text.color = normal_text_color
	stuff.text.shadow_color = text_shadow(stuff)
...
if (stuff):
	...
	stuff.text.color = key_text_color
	stuff.text.shadow_color = text_shadow(stuff)
	stuff.border.color = key_border_color
	stuff.border.width = 1
	stuff.etc.ellipse = ellipse
...
if (stuff):
	stuff.fill.color = KEY(normal_fill_color, -10)

## メニューのセパレータ
stuff = utils.create_stuff("menu_separator")
if (stuff):
	stuff.fill.color = ADD(key_border_color, 0)
...
if (stuff):
	...
	stuff.border.color = KEY(key_border_color, -40)
	stuff.border.width = 1
	stuff.etc.ellipse = ellipse
...
if (stuff):
	stuff.fill.color = KEY(key_fill_color, 20)
	stuff.border.color = KEY(key_border_color, 20)
	stuff.border.width = 1
...
edges = [
	ADD(key_fill_color, -30),
	ADD(key_fill_color, -10),
	ADD(key_fill_color, +30),
	ADD(key_fill_color, +50) ]
...
if (stuff):
	stuff.fill.color = ADD(normal_fill_color, -60)
...
exedit.group.color = KEY(key_fill_color, 30)
exedit.group.alpha = 0.15

# アイコン

icon = dark.exports.icon_manager
icon.change_color("EXEDIT_ICON_CL", CLR(0), KEY(key_fill_color, -50))
icon.change_color("EXEDIT_ICON_CL", CLR(255), CLR(0))
icon.change_color("EXEDIT_ICON_OP", CLR(0), KEY(key_fill_color, -50))
