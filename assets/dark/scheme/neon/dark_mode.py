...
## 通常状態
normal = Palette(
	utils.Tone(CLR(20, 20, 20)),
	utils.Tone(CLR(30, 30, 30)),
	key_tone,
...
# ボタンの配色をスタッフに適用します。
#
def button_apply(stuff, palette, ratio, text_ratio):
...
if (stuff):
	button_apply(stuff, button, ratio_disabled, 0.5)
...
#
def spin_apply(stuff, palette, ratio, text_ratio):
	stuff.fill.color = translucent(palette.fill.blend(0), ratio)
	stuff.fill.sub_color = translucent(palette.fill_sub.blend(0), ratio)
	stuff.fill.mode = fill_mode
	stuff.text.color = translucent(palette.text.blend(0), text_ratio)
...
# タブの配色をスタッフに適用します。
#
def tab_apply(stuff, palette, ratio, text_ratio):
	stuff.fill.color = translucent(palette.fill.blend(0), ratio)
	stuff.fill.sub_color = translucent(palette.fill_sub.blend(0), ratio)
	stuff.fill.mode = fill_mode
	stuff.text.color = translucent(palette.text.blend(0), text_ratio)
...
if (stuff):
	tab_apply(stuff, tab, ratio_disabled, 0.5)
...
#
def edit_apply(stuff, palette, ratio, text_ratio):
	stuff.fill.color = translucent(palette.fill.blend(0), ratio)
	stuff.text.color = translucent(palette.text.blend(0), text_ratio)
	stuff.text.shadow_color = text_shadow(stuff)
...
#
def listbox_apply(stuff, palette, ratio, text_ratio):
	stuff.fill.color = translucent(palette.fill.blend(0), ratio)
	stuff.text.color = translucent(palette.text.blend(0), text_ratio)
	stuff.text.shadow_color = text_shadow(stuff)
...
if (stuff):
	listbox_apply(stuff, listbox, 0, 0.5)
...
menu = Palette(
	normal.fill,
	normal.fill_sub,
	utils.Tone(CLR(100, 255, 255)),
	utils.Tone(CLR(100, 100, 200)),
	None)

## メニューの強調表示
menu_accent = Palette(
	utils.Tone(CLR(200, 50, 100)),
	utils.Tone(CLR(100, 100, 200)),
	utils.Tone(CLR(255)),
	utils.Tone(CLR(0, 0, 100)),
	None)

#
# メニューの配色をスタッフに適用します。
#
def menu_apply(stuff, palette, ratio, text_ratio):
	stuff.fill.color = translucent(palette.fill.blend(0), ratio)
	stuff.fill.sub_color = translucent(palette.fill_sub.blend(0), ratio)
	stuff.fill.mode = fill_mode
	stuff.text.color = translucent(palette.text.blend(0), text_ratio)
...
if (stuff):
	menu_apply(stuff, menu, 0.5, 0.5)

## メニューのホット状態
stuff = utils.create_stuff("menu_hot")
if (stuff):
	menu_apply(stuff, menu_accent, 0, 0)

## メニューのガター
stuff = utils.create_stuff("menu_gutter")
if (stuff):
	stuff.fill.color = translucent(key_tone.blend(0), 0)

## メニューのセパレータ
stuff = utils.create_stuff("menu_separator")
if (stuff):
	stuff.fill.color = translucent(key_tone.blend(0), 0.5)
...
#
def scrollbar_apply(stuff, palette, ratio):
	stuff.fill.color = translucent(palette.fill.blend(0), ratio)
	stuff.fill.sub_color = translucent(palette.fill_sub.blend(0), ratio)
	stuff.fill.mode = fill_mode
	stuff.border.color = translucent(palette.border.blend(0), ratio)
...
## トラックバーの基本配色
trackbar = Palette(
	utils.Tone(CLR(100, 0, 100)),
	utils.Tone(CLR(255, 0, 255)),
	None,
...
#
def trackbar_apply(stuff, palette, ratio):
	stuff.fill.color = translucent(palette.fill.blend(0), ratio)
	stuff.fill.sub_color = translucent(palette.fill_sub.blend(0), ratio)
	stuff.fill.mode = fill_mode
	stuff.border.color = translucent(palette.border.blend(0), ratio)
...
if (stuff):
	stuff.fill.color = translucent(CLR(0, 200, 255), 0.8)
	stuff.border.color = translucent(CLR(0, 200, 255), 0.6)
	stuff.border.width = 2

## トラックバーの選択部分
stuff = utils.create_stuff("trackbar_selected")
if (stuff):
	stuff.fill.color = translucent(CLR(0, 255, 0), 0.4)

## トラックバーの通常状態
stuff = utils.create_stuff("trackbar_normal")
if (stuff):
	trackbar_apply(stuff, trackbar, 0.2)
...
if (stuff):
	trackbar_apply(stuff, trackbar_accent, 0.2)

## トラックバーのプレス状態
stuff = utils.create_stuff("trackbar_pressed")
if (stuff):
	trackbar_apply(stuff, trackbar_accent, 0)

# エッジ

edges = [
	normal.fill.blend(-0.1),
	normal.fill.blend(-0.05),
	normal.fill.blend(0.05),
	normal.fill.blend(0.1),
	None ]
