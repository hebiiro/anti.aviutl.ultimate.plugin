...
## ペンギンの黒
normal = Palette(
	utils.Tone(CLR(25, 25, 30)),
	utils.Tone(CLR(45, 45, 45)),
	utils.Tone(CLR(250, 250, 245)),
	utils.Tone(CLR(100, 50, 0)),
	None)

## ペンギンの黄色～オレンジ
select = Palette(
	utils.Tone(CLR(215, 75, 0)),
	utils.Tone(CLR(255, 190, 5)),
	utils.Tone(CLR(255)),
...
accent = utils.Tone(CLR(255, 105, 0))
...
if (stuff):
	stuff.text.color = translucent(select.fill.blend(0), 0)
	stuff.text.shadow_color = text_shadow(stuff)
...
if (stuff):
	ratio = 0.75
	stuff.fill.color = translucent(select.fill.blend(0), ratio)
	stuff.fill.sub_color = translucent(select.fill_sub.blend(0), ratio)
	stuff.fill.mode = fill_mode
	stuff.text.color = translucent(select.text.blend(0), 0.5)
	stuff.text.shadow_color = text_shadow(stuff)
	stuff.border.color = translucent(select.border.blend(0), ratio)
	stuff.border.width = border_width
	stuff.etc.ellipse = ellipse
...
if (stuff):
	stuff.fill.color = translucent(normal.fill.blend(0.05), 0)
	stuff.text.color = translucent(normal.text.blend(0), 0)
	stuff.text.shadow_color = text_shadow(stuff)
...
if (stuff):
	stuff.fill.color = translucent(select.fill.blend(0.05), 0.2)
	stuff.text.color = translucent(select.text.blend(0), 0)
	stuff.text.shadow_color = text_shadow(stuff)
...
if (stuff):
	stuff.fill.color = translucent(normal.fill.blend(0), 0)
	stuff.border.color = translucent(normal.border.blend(0.1), 0)
	stuff.border.width = 1
...
edges = [
	normal.fill.blend(-0.4),
	normal.fill.blend(-0.2),
