...
namespace apn::filter_drag
{
	...
	struct Layout
	{
		...
		void init()
		{
			...
			{
				HWND arrow = ::GetDlgItem(setting_dialog, 4500 + i);
				HWND name = ::GetDlgItem(setting_dialog, 4400 + i);

				if (!::IsWindowVisible(arrow)) break;

				RECT arrow_rect;
				::GetClientRect(arrow, &arrow_rect);
				::MapWindowPoints(arrow, setting_dialog, (LPPOINT)&arrow_rect, 2);

				RECT name_rect;
				::GetClientRect(name, &name_rect);
		...
		int calc_bottom(const RECT& pane)
		{
			...
			HWND child = ::GetWindow(setting_dialog, GW_CHILD);
			while (child)
			{
		...
		// ドラッグ元フィルタを返します。
		//
		FilterHolder get_src_filter(POINT pos, ObjectHolder object) const
		{
		...
		// ドラッグ先フィルタを返します。
		//
		FilterHolder get_dst_filter(POINT pos, const ObjectHolder& object) const
		{
			for (size_t i = 0; i < panes.size(); i++)
			{
				if (!::PtInRect(&panes[i], pos)) continue;
