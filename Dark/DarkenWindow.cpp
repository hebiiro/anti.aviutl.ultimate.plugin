...
inline static struct {
	...
	BOOL init() {
		...
		::GetModuleFileNameW(g_instance, fileName, std::size(fileName));
...
void showSkinSelector(HWND hwnd)
{
	...
	getConfigFolderName(origin, std::size(origin));
	...
	if (id > 0 && id <= (int)fileNames.size())
	{
		...
		::StringCchCopy(fileName, std::size(fileName), _T("Skin"));
	...
	else if (id == ID_STATIC_EDGE_MODE)
	{
		...
		::MessageBox(hwnd, _T("このオプションはAviUtlを再起動したときに反映されます"), _T("DarkenWindow"), MB_OK);
	}
