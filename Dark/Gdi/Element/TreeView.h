	...
	inline struct TreeViewRenderer : Renderer
	{
		LRESULT onCustomDraw(Reflector* reflector, State* currentState) override
		{
			...
			case CDDS_ITEMPREPAINT:
				{
					HTHEME theme = skin::theme::manager.getTheme(skin::theme::THEME_TREEVIEW);
					auto state = skin::theme::manager.getState(theme, TVP_TREEITEM, TREIS_NORMAL);

					if (state && state->fillColor != CLR_NONE)
						cd->clrTextBk = state->fillColor;

					if (state && state->textForeColor != CLR_NONE)
						cd->clrText = state->textForeColor;
