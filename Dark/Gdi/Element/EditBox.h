...
{
	inline struct EditBoxRenderer : RendererNc
	{
...
//			if (!(options & ETO_IGNORELANGUAGE))
			{
				HTHEME theme = skin::theme::manager.getTheme(skin::theme::THEME_EDIT);

				// WM_CTLCOLORで設定されたstateIdを取得する。
				int stateId = hive.getStateId(currentState->hwnd);
