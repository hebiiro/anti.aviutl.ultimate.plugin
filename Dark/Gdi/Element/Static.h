...
{
	inline struct StaticRenderer : RendererNc
	{
		...
		BOOL onDrawEdge(State* currentState, HDC dc, LPRECT rc, UINT edge, UINT flags) override
		{
			...
			if (edge == EDGE_ETCHED && flags == BF_RECT)
			{
				...
				if (skin::theme::manager.onDrawThemeBackground(theme, dc, skin::theme::STAT_ETCHEDEDGE, 0, rc))
					return TRUE;
			}
...
//			if (!(options & ETO_IGNORELANGUAGE))
			{
				HTHEME theme = skin::theme::manager.getTheme(skin::theme::THEME_STATIC);
				if (skin::theme::manager.onExtTextOut(theme, dc, STAT_TEXT, 0, x, y, options, rc, text, c, dx))
					return TRUE;
			}
