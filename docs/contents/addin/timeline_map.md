...
* `タイムラインルーペを表示` ✏️『タイムラインルーペ』ウィンドウを表示します。
...
* `即時描画` ✏️タイムラインマップを即時に再描画するかどうかのフラグです。
	* チェックを外すと拡張編集ウィンドウの描画が優先され、逆にタイムラインマップの描画は遅延します。
...
* `文字を描画` ✏️文字を描画するかどうかのフラグです。
	* チェックを外すとテキスト(アイテム名)が表示されなくなり、描画が軽くなります。
* `影を描画` ✏️文字の影を描画するかどうかのフラグです。
* `最小幅` ✏️文字を描画するアイテムの最小幅です。単位はアイテムの高さを基準とした%です。
...
* `丸角モード` ✏️アイテムの丸角モードです。
	...
	* `相対` ✏️`丸角サイズ`の単位が最も短い辺を基準とした%になります。
	* `個別` ✏️`丸角サイズ`の単位が個別の辺を基準とした%になります。
...
* 左ドラッグで表示フレーム数と表示レイヤー数を変更できます。
* 右クリックで`コンテキストメニュー`を表示できます。

### 📝コンテキストメニュー

* `タイムラインルーペの設定` ✏️`タイムラインルーペの設定`ダイアログを表示します。

## 🎨『タイムラインルーペの設定』ダイアログ

### 🏷️再描画対象

* `タイムライン` ✏️タイムライン上にマウスカーソルがあるときに再描画します。
* `タイムラインマップ` ✏️タイムラインマップ上にマウスカーソルがあるときに再描画します。

### 🏷️描画領域

* `フレーム数` ✏️描画するフレーム数を指定します。
* `レイヤー数` ✏️描画するレイヤー数を指定します。
	* 小さくする程、アイテムが大きく描画されます。
