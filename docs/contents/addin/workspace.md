...
* メインウィンドウのメニューで`その他`➡`ワークスペース化`をクリックすると`ワークスペース化メニュー`が表示されます。
...
1. まず、`『ワークスペース化』の設定`で`『再生中にプレビューを最大化』機能を有効化`にチェックを入れます。
...
1. フローティングウィンドウの表示位置をリセットします。
1. リセットされたフローティングウィンドウはモニタの中央に表示されます。
...
* `ダイアログメッセージを処理する` ✏️モードレスダイアログのダイアログメッセージを処理します。
	* タブキーでのフォーカス移動ができるようになります。
