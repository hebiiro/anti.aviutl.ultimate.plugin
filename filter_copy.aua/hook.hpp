	...
	inline struct HookManager
	{
		...
		struct Prep {
			...
			BOOL init()
			{
				...
				// テンポラリフォルダのパスを取得します。
				auto buffer_length = ::GetTempPathA(0, 0);
				temp_folder_path.resize(buffer_length);
				auto length = ::GetTempPathA(temp_folder_path.size(), temp_folder_path.data());
		        temp_folder_path.resize(length);
