...
namespace apn::exedit_tweaker
{
	...
	inline struct HookManager
	{
		...
		BOOL init()
		{
			...
			if (hive.gradient_fill.enable) my::hook::attach(gradient_fill, exedit + 0x00036a70);
		...
		struct {
			...
			inline static void CDECL hook_proc(HDC dc, const RECT *rc, BYTE r, BYTE g, BYTE b, BYTE gr, BYTE gg, BYTE gb, int gs, int ge)
			{
				...
				// 矩形の枠を描画します。
				RECT rcFrame = *rc;
				hive.gradient_fill.edge.outer.draw(dc, &rcFrame);
				hive.gradient_fill.edge.outer.deflate_rect(&rcFrame);
				hive.gradient_fill.edge.inner.draw(dc, &rcFrame);
			}
