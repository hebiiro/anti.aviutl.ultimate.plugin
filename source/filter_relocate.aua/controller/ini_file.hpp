...
namespace apn::filter_relocate::controller
{
	...
	struct ini_file_t
	{
		...
		struct section_t
		{
			...
			// このセクションのクローンを返します。
			//
			std::shared_ptr<section_t> clone()
			{
				return std::make_shared<section_t>(*this);
			}
		};

		std::vector<std::string> keys;
		std::unordered_map<std::string, std::shared_ptr<section_t>> sections;

		//
		// セクションを作成して追加します。
		//
		std::shared_ptr<section_t> create_section(const std::string& name)
		{
			return add_section(name, std::make_shared<section_t>());
		}

		//
		// セクションを追加します。
		//
		std::shared_ptr<section_t> add_section(const std::string& name, const std::shared_ptr<section_t>& section)
		{
		...
		//
		std::shared_ptr<section_t> get_section(const std::string& name)
		{
			auto it = sections.find(name);
		...
		//
		std::shared_ptr<section_t> set_section(const std::string& name, const std::shared_ptr<section_t>& section)
		{
		...
		BOOL read_file(const std::string& ini_file_name)
		{
			...
			{
				// 現在読込中のセクションです。
				std::shared_ptr<section_t> current_section;
				...
				if (!stream)
				{
					...
					view::main_window.message_box(my::format(
			...
			{
				// メッセージボックスでユーザーに通知します。
				view::main_window.message_box(my::ws(error.what()));
			}
